...
- [x] `hierarchy/`에서 recursive hierarchy traversal 을 `CopcHierarchyNode[]`로 안정적으로 변환
...
## Phase 2 - Cesium Rendering Pipeline
...
- [x] unit test 정비
- [x] integration test가 필요한 흐름 선별 및 추가
- [x] type check, build 검증 정리
- [x] coverage 기준 수립 및 자동화
...
- Screen Space Error 기반 LOD refinement
- WebWorker offload
- Rust + WASM 범위를 metadata / hierarchy parsing 까지 확장

## Next Priorities

우선순위는 아래 순서로 진행한다.

1. selection heuristic 고도화 (예: screen space error)
2. WebWorker offload 검토
3. Rust decoder를 full COPC parsing 단계까지 확장
